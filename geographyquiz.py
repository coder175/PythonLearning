...
def GeographyQuiz():
    ...
    t.sleep(1)
    wantInstr = input(
        f.apply("Would you like to read the Instructions? Please type", "green") + f.apply(" Yes",
                                                                                           "bold green") + f.apply(
            " or ", "green") + f.apply("No. ", "bold green"))
    wantInstr.replace(" ", "")
    while wantInstr.lower() not in ("yes", "no", "y", "n"):
        wantInstr = input(
            f.apply("You typed '" + wantInstr + "'. Please type either Yes or No to skip the instructions.",
    ...
    print("")
    print(f.apply(f" Total Questions Correct: {correct} ", "bold white red_bg"))
    t.sleep(0.7)
    print(f.apply(f" Total Points: {points} ", "bold white red_bg"))
    t.sleep(0.7)
    print(f.apply(f" Total Average: {int((sum(grades)) / len(grades))}% ", "bold white red_bg"))
    t.sleep(1.2)
    ...
    if len(incorrectQ) > 0:
        ...
        # Find % Similar:
        redemptionSimilar = d.SequenceMatcher(None, re.sub(r"[^a-zA-Z]", "", redemptionAnswer.lower()),
                                              userRedemptionAns)
        percentSimilar = redemptionSimilar.ratio() * 100
        grades.append(percentSimilar)
        loading()
        if percentSimilar >= 85.0:
            print(f.apply(" Redemption Question Correct!", "bold green"))
            t.sleep(1.5)
            if percentSimilar != 100.0:
                print("Answer: " + redemptionAnswer + ".")
                t.sleep(0.8)
            print("Accuracy: " + f.apply(str(int(percentSimilar)), "bold green"))
            correct += 1.0
            points += 1
        elif percentSimilar <= 65.0:
            print(f.apply("Redemption Question Incorrect.", "red bold"))
            t.sleep(0.8)
            print("Answer: " + redemptionAnswer + ".")
            t.sleep(0.8)
            print("Accuracy: " + f.apply(str(int(percentSimilar)), "bold red"))
            ...
            print(f.apply("Redemption Question Partially Correct!", "bold yellow"))
            t.sleep(1.5)
            print("Answer: " + redemptionAnswer + ".")
            t.sleep(1.5)
            print("Accuracy:" + f.apply(str(int(percentSimilar)), "bold yellow"))
        ...
        print("")
        t.sleep(0.75)
        print("Including the Redemption Question, these are you results: ")
        print("")
        print(f.apply(f" Total Questions Correct: {correct} ", "bold white red_bg"))
        t.sleep(0.7)
        print(f.apply(f" Total Points: {points} ", "bold white red_bg"))
        ...
        print("You didn't get any questions wrong, so you don't have a redemption Question.")
        t.sleep(0.5)
    end = input(f.apply("If you want to play again, type P, or type X to exit. ", "Bold Green Green_bg"))
    end.replace(" ", "")
    while end.lower() not in ("x", "p"):
        wantInstr = input(f.apply("You typed '" + wantInstr + "'. Please type either X or P to either play again, "
                                                              "or exit.", "bold yellow yellow_bg"))
    if end == "x":
        exit()
    else:
        t.sleep(1)
